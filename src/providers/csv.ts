...
export function memoryToCells(row: { memory: MemoryRecord; vector: number[] | null }): string[] {
...
function csvLine(row: { memory: MemoryRecord; vector: number[] | null }): string {
...
function rewriteFile(filePath: string, rows: Array<{ memory: MemoryRecord; vector: number[] | null }>): void {
  const tmp = `${filePath}.tmp`;
  const body = rows.length
    ? rows.map((row) => memoryToCells(row)).map(csvLineText).join("")
    : "";

...
import { renameSync, writeFileSync, existsSync, readFileSync, mkdirSync } from "node:fs";
import { dirname, join } from "node:path";
import {
  HEADER, csvParse, csvLineText, memoryToCells, rowToMemory, parseVector,
} from "./csv.ts";
...
export class RoxifiedCSVProvider implements FortuneProvider {
  ...
  async addMemory(memory: MemoryRecord, vector: number[] | null): Promise<void> {
    ...
    const index = this.rows.findIndex((candidate) => candidate.memory.id === memory.id);
...
let cached: {
  ...
  decodePngToBinary(png: Buffer): Promise<{ buf: Buffer; meta: { name: string } }>;
} | null = null;

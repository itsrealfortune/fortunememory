...
export class PgliteProvider implements FortuneProvider {
  ...
  static async create(options: { dataDir?: string } = {}): Promise<PgliteProvider> {
  ...
  private memoryToParams(memory: MemoryRecord, vector: number[] | null, dollar = true): unknown[] {
    void dollar;
    return [
      memory.id,
      memory.type,
  ...
  async addMemory(memory: MemoryRecord, vector: number[] | null): Promise<void> {
    ...
    await client.query(
      `INSERT INTO fortune_memories (${COLUMNS.join(", ")}) VALUES (${COLUMNS.map((_, index) => placeholder(index + 1)).join(", ")})
       ON CONFLICT (id) DO UPDATE SET ${COLUMNS.slice(1).map((column, index) => `${column} = ${placeholder(index + 2)}`).join(", ")}`,
  ...
  async count(): Promise<{ active: number; forgotten: number }> {
    ...
    const row = (result.rows?.[0] ?? {}) as { active?: string | number; forgotten?: string | number };
...
async function importPGlite(dataDir?: string | undefined): Promise<{
  ...
  query<T = Record<string, unknown>>(sql: string, params?: unknown[]): Promise<{ rows?: T[]; changes?: number; rowCount?: number }>;
  close(): Promise<void>;
}> {
...
export class MysqlProvider implements FortuneProvider {
  readonly name = "mysql";
  private conn: { query(sql: string, params?: unknown[]): Promise<unknown[]>; end(): Promise<void>; execute(sql: string, params?: unknown[]): Promise<unknown>; } | null = null;
  ...
  async init(): Promise<void> {
    ...
    this.conn = await mysql.createConnection(this.dsn) as unknown as NonNullable<typeof this.conn>;
  ...
  private memoryToParams(memory: MemoryRecord, vector: number[] | null): unknown[] {
    return [
      memory.id,
      memory.type,
      memory.content,
...
function rowToMemory(row: Record<string, unknown>): MemoryRecord {
  ...
  return {
    ...
    tags: Array.isArray(row.tags) ? row.tags.map(String) : typeof row.tags === "string" ? (JSON.parse(row.tags) as string[]) : [],

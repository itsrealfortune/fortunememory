...
const TRUST_RANK = (value: string): number => {
  ...
  return rank < 0 ? (SOURCE_TRUST_LEVELS as readonly string[]).indexOf("owner") : rank;
};
...
export class FortuneMemoryManager {
  ...
  async remember(input: MemoryDraft): Promise<MemoryRecord> {
    ...
    const duplicate = await this.findByContentHash(memory.contentHash ?? "", memory.scope);
  ...
  private async findByContentHash(
    ...
    for await (const row of this.provider.iterate(false)) {
      ...
      const existingHash = existing.contentHash ?? memoryContentHash(existing.content);
      if (existingHash === hash && scopeMatches(existing.scope, scope)) return existing;
    }
    return null;
  }
  ...
  async stats(): Promise<{ active: number; forgotten: number; vectorModel: string }> {
  ...
  async list(
    ...
    const candidates = await this.filterRows(this.iterActive(), options, undefined);
    candidates.sort((left, right) =>
      compareDate(right, left, (m) => m.occurredAt || m.validFrom || m.createdAt),
    );
    return candidates.slice(0, limit).map((row) => row.memory);
  }

  private async *iterActive(): AsyncGenerator<{ memory: MemoryRecord; vector: number[] | null }> {
    for await (const row of this.provider.iterate(false)) {
  ...
  async findConflicts(
    input: { content: string; type: string; scope: string; validFrom?: string | null; validTo?: string | null },
    options: { threshold?: number; limit?: number } = {},
  ): Promise<ConflictCandidate[]> {
    if (!["fact", "preference", "decision"].includes(String(input.type))) return [];
  ...
  private async rankConflicts(
    ...
    return candidates.slice(0, limit).map((candidate) => ({
      memory: { ...candidate.memory, similarity: Number(candidate.vectorScore.toFixed(4)) },
  ...
  async getContext(
  ...
  ): Promise<{ query: string; context: string; usedChars: number; count: number }> {
    ...
    const context = lines.length ? header + lines.join("\n") : header + "(aucun souvenir correspondant)";
  ...
  private async filterRows(
    ...
    const maxSensitivityRank = SENSITIVITY_RANK(options.maxSensitivity ?? "restricted");
    const minTrustRank = options.minSourceTrust ? TRUST_RANK(options.minSourceTrust) : 0;
    ...
    for await (const row of iter) {
      ...
      if (options.excludeContent && memory.content === options.excludeContent) continue;
      rows.push(row);
    }
    return rows;
  }
  ...
  private scoreLexical(
    ...
    for (const row of rows) {
      ...
      if (!overlap) continue;
      const lexicalScore = Math.min(1, overlap / Math.max(1, meaningful.length));
    ...
    return scored
      .sort(
        (left, right) =>
          ...
          TRUST_RANK(right.hit.memory.sourceTrust) - TRUST_RANK(left.hit.memory.sourceTrust),
      )
      .map((entry) => entry.hit)
      .slice(0, limit);
  }
  ...
  private async scoreVector(
    ...
    candidates.sort(
      (left, right) =>
        ...
        TRUST_RANK(right.memory.sourceTrust) - TRUST_RANK(left.memory.sourceTrust),
    );
...
const STOPWORDS = new Set([
  "le", "la", "les", "de", "des", "du", "un", "une", "et", "est", "es", "qui",
  "que", "quel", "quelle", "ai", "a", "je", "tu", "il", "elle", "on", "nous",
  "vous", "ils", "c", "d", "l", "s", "t", "en", "au", "aux", "ce", "cette",
  "ces", "sur", "pour", "avec", "dans", "par", "pas", "plus", "moins", "the",
  "a", "an", "is", "it", "of", "to", "and", "in", "on", "his", "her", "its",
  "my", "your", "mes", "elle", "lui", "quoi", "comment", "pourquoi", "mon",
  "ma", "son", "ses", "leur", "leurs", "suis", "sommes", "sont",
]);
...
function fuseRankings(
  ...
  const fused = new Map<
    string,
    { memory: MemoryRecord; score: number; lexicalRank: number | null; vectorRank: number | null; vectorSimilarity: number | null }
  >();
  ...
  lexical.forEach((hit, index) => add(hit, index + 1, "lexical"));
  vector.forEach((hit, index) => add(hit, index + 1, "vector"));

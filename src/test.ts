...
function autoInit(provider: FortuneProvider): FortuneProvider {
	...
	return {
		...
		addMemory: (memory, vector) =>
			queue(() => provider.addMemory(memory, vector)),
		updateStatus: (id, status, at) =>
			queue(() => provider.updateStatus(id, status, at)),
		getMemory: (id, includeForgotten) =>
			queue(() => provider.getMemory(id, includeForgotten)),
...
async function runSuite(
	...
	try {
		...
		check(
			"remember renvoie un record complet",
			Boolean(fox.id && fox.contentHash),
		);
		...
		check(
			"search lexical trouve Fox",
			lexical.some((m) => m.content.includes("Fox")),
		);

		const hybrid = await manager.search("soeur de coeur café", {
			scope: "discord/dm/42",
		});
		check(
			"search hybrid trouve Fox",
			hybrid.some((m) => m.id === fox.id),
		);
		check(
		...
		const publicOnly = await manager.search("wifi", {
			maxSensitivity: "public",
		});
		check(
			"maxSensitivity public masque le private",
			!publicOnly.some((m) => m.id === secret.id),
		);
		...
		check(
			"findConflicts détecte le quasi-doublon",
			conflicts.some((c) => c.memory.id === fox.id),
		);

		check(
			"findConflicts ignore les types non listés",
			(
				await manager.findConflicts({
		...
		check(
			"stats comptent 2 actifs / 1 oublié",
			stats.active === 2 && stats.forgotten === 1,
		);
...
async function runSuiteOptional(
	...
	try {
		...
		if (
			...
			/dépendance optionnelle/.test(error.message)
		) {
			console.log(`\n=== ${label} ===\n  ⊘ skip (${error.message})`);
			return;
		}
...
export async function runTests(): Promise<void> {
	...
	const unrelated = await encoder.encode(
		"réplication base de données postgres",
	);
	check(
		"cosine(similaire) > cosine(étranger)",
		cosineSimilarity(close, similar) > cosineSimilarity(close, unrelated),
	);
	check(
		"norme unitaire",
		Math.abs(
			Math.sqrt(close.reduce((sum, value) => sum + value * value, 0)) - 1,
		) < 1e-6,
	);
	const hashA = memoryContentHash("abc");
	check(
		"contentHash stable et discriminant",
		hashA === memoryContentHash("abc") && hashA !== memoryContentHash("abd"),
	);
	...
	await runSuiteOptional("Roxify provider (PNG)", (dir) =>
		roxifyProviderFactory(dir),
	);
	await runSuiteOptional("RoxifiedCSV provider (PNG)", (dir) =>
		roxifiedCsvProviderFactory(dir),
	);
	...
	try {
		...
		const reopened = new FortuneMemoryManager(
			autoInit(jsonProviderFactory(dir)),
		);
		const persisted = await reopened.search("persist me", {
			retrieval: "lexical",
		});
		check(
			"json survive au reopen",
			persisted.some((m) => m.content === "persist me"),
		);

		const sqDir = mkdtempSync(join(tmpdir(), "fortune-sqlite-"));
		const sq1 = new FortuneMemoryManager(
			autoInit(sqliteProviderFactory(sqDir)),
		);
		await sq1.remember({ content: "sqlite persist test", scope: "personal" });
		const sq2 = new FortuneMemoryManager(
			autoInit(sqliteProviderFactory(sqDir)),
		);
		check(
			"sqlite survive au reopen",
			(await sq2.search("sqlite persist test", { retrieval: "lexical" }))
				.length === 1,
		);

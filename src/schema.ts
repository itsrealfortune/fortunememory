...
export const SENSITIVITY_LEVELS = ["public", "personal", "private", "restricted"] as const;
...
function optionalDate(value: unknown, field: string): string | null | undefined {
  if (value === undefined || value === null || value === "") return value === "" ? undefined : value ?? undefined;
 ...
 */
export function normalizeMemory(input: MemoryDraft, now = new Date()): MemoryRecord {
  ...
  const scope = String(input.scope ?? "personal").trim().slice(0, 200);
  ...
  return {
    ...
    summary,
    source: {
      kind: String(input.sourceKind ?? "manual").trim().slice(0, 50) || "manual",

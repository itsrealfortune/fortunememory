...
class OllamaEmbeddings implements OllamaLikeProvider {
  ...
  async encode(text: string): Promise<number[]> {
    ...
    if (!Array.isArray(payload.embedding)) throw new Error("ollama embeddings: payload invalide");
...
class OpenAICompatibleEmbeddings implements OllamaLikeProvider {
  ...
  async encode(text: string): Promise<number[]> {
    ...
    if (!Array.isArray(embedding)) throw new Error("embeddings: payload invalide");
    return embedding;
  }
}
...
export function resolveVectorProvider(
  ...
  const name = String(spec || env.FORTUNE_EMBEDDINGS || env.OPENSELF_EMBEDDINGS || "feature-hash").toLowerCase();
  switch (name) {
    case "feature-hash":
    case "hash":
    case "local":
    ...
    case "ollama":
      return new OllamaEmbeddings({
        baseUrl: env.FORTUNE_OLLAMA_URL || env.OPENSELF_OLLAMA_URL || "http://127.0.0.1:11434",
        model: env.FORTUNE_EMBEDDINGS_MODEL || env.OPENSELF_EMBEDDINGS_MODEL || "nomic-embed-text",
      });
    ...
    case "openai": {
      ...
      if (!apiKey) {
        throw new Error("openai-compatible embeddings exige FORTUNE_EMBEDDINGS_API_KEY");
      }
    ...
    default:
      throw new Error(`embeddings inconnu : '${name}' (feature-hash | ollama | openai-compatible)`);
  }
}

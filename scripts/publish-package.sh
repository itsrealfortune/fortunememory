#!/usr/bin/env bash
# Release locale de fortunememory : tag fortunememory-v<version> + release GitHub.
# Le workflow publish-fortunememory.yml publie sur npm depuis ce tag.
# À lancer depuis .opencode/plugins/fortunememory/.
set -euo pipefail
...
git tag -a "${TAG}" -m "fortunememory ${TAG}"
git push origin "${TAG}"
